...
class KinematicCarMotionModel:
    ...
    def compute_changes(self, states, controls, dt, alpha_threshold=1e-2):
        ...
        nz_alpha = np.abs(alpha) >= alpha_threshold
        zero_alpha = ~nz_alpha
        
        # Case 1: Zero steering angle 
        changes[zero_alpha, 0] = vel[zero_alpha] * np.cos(theta[zero_alpha]) * dt
        changes[zero_alpha, 1] = vel[zero_alpha] * np.sin(theta[zero_alpha]) * dt
        changes[zero_alpha, 2] = 0.0
        
        # Case 2: Non-zero steering angle
        if np.any(nz_alpha):
            dtheta = (vel[nz_alpha] / self.car_length) * np.tan(alpha[nz_alpha]) * dt
            theta_nz = theta[nz_alpha]
            vel_nz = vel[nz_alpha]
            
            #  dx and dy 
            changes[nz_alpha, 0] = (self.car_length / np.tan(alpha[nz_alpha])) * \
                                        (np.sin(theta_nz + dtheta) - np.sin(theta_nz))
            changes[nz_alpha, 1] = (self.car_length / np.tan(alpha[nz_alpha])) * \
                                        (-np.cos(theta_nz + dtheta) + np.cos(theta_nz))
            changes[nz_alpha, 2] = dtheta
    ...
    def apply_deterministic_motion_model(self, states, vel, alpha, dt):
        ...
        # BEGIN QUESTION 1.3
        controls = np.tile([vel, alpha], (n_particles, 1)) # by hint
        
        # Compute changes using the method from Q1.2
        changes = self.compute_changes(states, controls, dt)
        states[:] = states + changes
        states[:, 2] = (states[:, 2] + np.pi) % (2 * np.pi) - np.pi
        states[:, 2] = np.where(states[:, 2] == -np.pi, np.pi, states[:, 2])
    ...
    def apply_motion_model(self, states, vel, alpha, dt):
        ...
        # BEGIN QUESTION 1.4
        noisy_vel = vel + np.random.normal(0, self.vel_std, n_particles)
        noisy_alpha = alpha + np.random.normal(0, self.alpha_std, n_particles)
        
        controls = np.column_stack([noisy_vel, noisy_alpha])
        changes = self.compute_changes(states, controls, dt)
        states[:] = states + changes
        
        states[:, 0] += np.random.normal(0, self.x_std, n_particles)
        states[:, 1] += np.random.normal(0, self.y_std, n_particles)
        states[:, 2] += np.random.normal(0, self.theta_std, n_particles)
