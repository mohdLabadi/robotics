...
Before starting Homework 1, make sure you’ve followed the instructions in `pre_hw_vm_setup.md` to set up your workspace.  
Your homework assignment should be placed inside the **same** `src/` folder of your ROS workspace. Your directory structure should look like this:
```
homework_ws 
├── devel 
├── build 
├── src 
    ...
    ├── hw1_introduction 
```
Note: Some hints are provided in Hints_for_HW1.pdf. You may refer to them as needed.
...
```
chmod +x ~/homework_ws/src/hw1_introduction/test/test_prime_number_small.py
```
...
roslaunch introduction prime_number.launch input_number:=11
roslaunch introduction prime_number.launch input_number:=12
```
...
roslaunch cs4750 teleop.launch
```
If you run into the error "Resource not found: The following package was not found in...", this is because your `dependencies_ws` is not properly sourced. You will have to re-build and re-source your workspace to dependencies_ws. In `dependencies_ws`, run the following command (remember to run all these commands in the **same terminal**):
```
catkin clean
catkin build
source ~/homework_ws/devel/setup.bash
...
Then, to visualize the simulation, open up a separate terminal and run:
```
rosrun rviz rviz -d ~/homework_ws/src/cs4750/config/default.rviz
```
...
This will launch RViz with a configuration that has all the right topics visualized. A window like this should appear:
 ...
 <figure>
  <img src="./EXAMPLE_RVIZ_IMAGE_DO_NOT_SUBMIT.png" alt="alt text" width="1000"/>
...
```
  > Run the above configuration with the figure_8.txt and crown.txt plan files. For both plans, please scale down the y-axes so that the patterns for the norms figures and then save the figures produced by pose_listener for submission: locations.png and norms.png. In total, you should have 4 image files for submission: `figure_8_locations.png`, `figure_8_norm.png`, `crown_locations.png`, and `crown_norm.png`. *Important:* Make sure to restart the simulator before generating figures for each of the two plan files!

  > Optional: Let your artistic side shine and create your own plan file! If you do, please submit the plan file (and any code you may have used to generate it), as well as the locations.png producted by pose_listener. *Plan file format*: The first line of your plan file should specify the initial pose of the car (x,y, heading angle), while every line thereafter should specify a (forward velocity, steering angle) pair. Additionally, make sure that there are no empty lines in your plan file (otherwise you may get garbage output).
...
### Submission 
zip hw1_introduction folder and submit to Gradescope. You must zip your folders along with the top folder as the wrapper folder (hw1_introduction). IMPORTANT: Make sure you remove -main from the name of the folder before zipping.
```
$ zip -r <name of zip> hw1_introduction/
```

#### How to add figures to your Markdown submission
Please answer all the write-up questions in the file:
hw1_introduction/writeup/README.md. To include figures in your submission, save your image files in the 'writeup' folder, then add the images to your README using the following Markdown format: `![The title of your image](imageName.png)`.
