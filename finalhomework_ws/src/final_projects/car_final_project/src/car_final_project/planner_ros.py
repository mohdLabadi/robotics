...
class PlannerROSWithControl(PlannerROS):
    ...
    def send_path(self, path_states):
        ...
        frame_id = "map"
        try:
            # map_info might be a dict or an object; be defensive
            if isinstance(self.map_info, dict) and "frame_id" in self.map_info:
                frame_id = self.map_info["frame_id"]
            elif hasattr(self.map_info, "frame_id"):
                frame_id = self.map_info.frame_id
        except Exception:
            pass
        ...
        for s in path_states:
            ps = PoseStamped()
            ps.header = hdr
            ps.pose = utils.particle_to_pose(s)
            path_msg.poses.append(ps)

        try:
            resp = self.controller(path_msg, 2.2)
        except rospy.ServiceException as e:
            rospy.logerr("FollowPath service call failed: %s", e)
            raise
