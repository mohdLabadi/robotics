...
class SE2Problem(PlanarProblem):
    ...
    def compute_distance(self, q1, q2):
        ...
        q2_2d = np.atleast_2d(q2)
        ### BEGIN QUESTION 4 #####################
        return dubins.path_length(q1_2d, q2_2d, self.curvature)
        ### END QUESTION 4 #######################
...
class JointSpace(object):
    ...
    def compute_distance_rrt(self, start_config, goal_config):
        ...
        '''
        ### BEGIN QUESTION 6 #####################
        velocities = [1, 1.2, 0.8, 0.75, 1.05, 0.9]
        distance = 0
        for i in range(6):
            abs_dist = np.abs(start_config[:,i] - goal_config[:,i])
            min_dist = np.minimum(abs_dist, np.abs(2*np.pi - abs_dist))/velocities[i]
            distance = np.maximum(distance, min_dist)
        return np.ndarray.item(distance)
        ### END QUESTION 6 #######################
    ...
    def cost_to_go(self, config, goal_config):
        ...
        for i in range(6):
            # Angular distance calculation
            # Source: https://stackoverflow.com/questions/1878907/how-can-i-find-the-smallest-difference-between-two-angles-around-a-point 
            dist = config_2d[:,i] - goal_config_2d[:,i]
            dist = np.abs((dist + np.pi) % (2*np.pi) - np.pi)
            heuristic += dist
        epsilon = 1e-5
        return np.maximum(0,heuristic-epsilon) # unfortunately needed due to precision issues w/ float modulus
        ### END QUESTION 5 #######################
