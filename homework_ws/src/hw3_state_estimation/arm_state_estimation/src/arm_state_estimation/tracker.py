...
from arm_state_estimation.particle_filter import ParticleFilter
...
class CylinderTracker:
    def __init__(self):
        ...
        # Question 1.4: Initial values of parameters to tune
        ################################################################
        ...
        self.plot_path = rospack.get_path('arm_state_estimation')
    ...
    def add_noise(self, x_true, y_true, radius, current_frame):
        ...
        if self.rng.uniform() > 0.7:
            x = x_true + int(self.rng.normal(0, 50, 1))
            y = y_true + int(self.rng.normal(0, 50, 1))
        else:
            x = x_true
            y = y_true
