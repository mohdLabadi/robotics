...
from car_state_estimation.motion_model import KinematicCarMotionModelROS
from car_state_estimation.sensor_model import LaserScanSensorModelROS
from arm_state_estimation.resampler import LowVarianceSampler
...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        particles[:, 0] = self.rng.normal(x, self.x_std, n_particles)
        particles[:, 1] = self.rng.normal(y, self.y_std, n_particles)
        particles[:, 2] = self.rng.normal(theta, self.theta_std, n_particles)
...
class ParticleFilter:
    ...
    def __init__(self, motion_params=None, sensor_params=None, **kwargs):
        ...
        self.pose_pub = rospy.Publisher(
            rospy.get_param("~inferred_pose", "/car/particle_filter/inferred_pose"),
            PoseStamped,
            queue_size=1
        )
        
        self.particle_pub = rospy.Publisher(
            rospy.get_param("~particles","/car/particle_filter/particles"),
            PoseArray,
            queue_size=1
        )
        
        self.pub_odom = rospy.Publisher(
            rospy.get_param("~odom", "/car/particle_filter/odom"),
            Odometry,
            queue_size=1
        )
            ...
            if self.particle_pub.get_num_connections() > 0:
                with self.state_lock:
                    if self.particles.shape[0] > self.n_viz_particles:
                        ...
                        proposal_indices = self.particle_initializer.rng.choice(
