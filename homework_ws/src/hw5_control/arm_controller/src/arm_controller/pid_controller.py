...
class PIDController():
    ...
    def get_control(self):
        ...
        self.d_error = (error - self.p_error) * self.frequency
        
        #  accumulate error
        self.i_error += error / self.frequency
        
        # PID control law: u = -(Kp*e + Ki*integral + Kd*derivative)
        u = -(self.kp * error + self.ki * self.i_error + self.kd * self.d_error)
        self.p_error = error
