...
Note: Some hints are provided in hw5_hints.pdf. You may refer to them as needed.


## Code Overview

We decompose the problem of computing the robot’s actions into two components: motion planning and control. A motion planning algorithm generates an ideal **reference path**, while a feedback controller uses the robot’s current state (e.g. from the `car_state_estimation` package) to follow this reference path. In this project, we will use a series of pre-programmed paths.
...
## \# Q2. PID Pure Pursuit Controller (20 points)
...
The PID pure pursuit controller is controlled by the gains $K_p$, $K_i$, and $K_d$. First, complete the `get_ros_params()` function in `car_controller/control_ros.py` to load these values from the ROS parameter server. We have provided initial values in `config/parameters.yaml`, and you are free to tune them. It’ll be up to you to tune the gains $K_p, K_i, K_d$ to achieve good path tracking performance. You may use the Ziegler–Nichols method discussed in class for tuning.
...
## \# Q3. PID controller in WidowX250 (10 points)
...
Submit both written and coding assignments on Gradescope. Ensure that you have justified your parameter tuning in your writeup, and included all three plots. Please place your justification and plots in the /hw5_control/writeup folder. The written assignment is for graduate students only, no submission is required for undergraduate students. Please note that the written assignment for graduate students has a separate Gradescope submission page (Homework 5 [Written]).
