...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        steering_angles = np.linspace(self.min_alpha, self.max_alpha, self.K)
        controls[:, :, 1] = steering_angles[:, np.newaxis]
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        final_states = rollouts[:, -1, :2]
        reference_pos = reference_xyt[:2]
        distances = np.linalg.norm(final_states - reference_pos, axis=1)
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # BEGIN SOLUTION "QUESTION 4.3" ALT="raise NotImplementedError"
        collision_counts = np.sum(rollout_collisions, axis=1)
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        # Hint: Note that the reference velocity has already been stored in self.sampled_controls.
        # BEGIN SOLUTION "QUESTION 4.4" ALT="raise NotImplementedError"
        best_idx = np.argmin(costs)
        control = self.sampled_controls[best_idx, 0, :]
        return control
        # END SOLUTION
