...
def compute_position_in_frame(p, frame):
    ...
    x, y, _ = p
    x_f, y_f, theta_f = frame

    dx = x - x_f
    dy = y - y_f

    cos_th = np.cos(theta_f)
    sin_th = np.sin(theta_f)
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            # Vectorized computation of distances from current pose to all waypoints
            # path_xytv[:, :2] extracts all x, y positions from the path
            distances = np.linalg.norm(path_xytv[:, :2] - np.array([x, y]), axis=1)
            
            # Find the index of the closest waypoint
            closest_idx = np.argmin(distances)
            
            # Only consider waypoints from the closest one onward (to avoid going backwards)
            # Compute distances from current pose to waypoints starting from closest_idx
            distances_from_closest = distances[closest_idx:]
            
            # Find indices where distance > distance_lookahead
            # We add closest_idx to convert back to the original path indices
            lookahead_indices = np.where(distances_from_closest > distance_lookahead)[0]
            
            if len(lookahead_indices) > 0:
                # Return the first index that exceeds the lookahead distance
                return closest_idx + lookahead_indices[0]
            else:
                # No waypoint exceeds lookahead distance, return the last waypoint
