        ...
        while len(queue) > 0:
            ...
            for neighbor, w in self.rm.graph[entry.node].items():
                ...
                if not expanded[neighbor]:
                            g_neighbor = entry.cost_to_come + weight                 
                            f_neighbor = g_neighbor + h
                            queue.push(QueueEntry(f_neighbor, next(c), neighbor, entry.node, g_neighbor))
                ### END QUESTION 1.2 #####################
...
class RRTPlanner(object):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(xs_rand.shape[0]):
                ...
                if x_new is not None:
                    new_vid = self.tree.AddVertex(x_new)
                    self.tree.AddEdge(x_near_id, new_vid)
                    new_eid.append(new_vid)
    ...
    def extend(self, x_near, x_rand):
        ...
        step_size = self.eta * distance
        
        v_normalized = v / distance
        v_scaled = v_normalized * step_size
        
        x_new = x_near + v_scaled
